...
date: " created 4 October 2021, updated 28. november 2022"
...
```{r relabel column}
names(Western_Europe)[4] <- "homocides_per_100k"
```
...
ggplot(data = Western_Europe) + 
  geom_line(aes(x = Year, y = homocides_per_100k, color = Entity)) + 
  labs(title = "European Homocides through the years", y = "Homocides In Europe", x = "year")
...
```{r eval = FALSE}
# YOUR CODE HERE:
monarchs_data <- read_csv2("data/Kongerækken.csv")

monarchs_mid <- monarchs_data%>% 
  mutate(mid_year = End_Reign-(End_Reign-Start_Reign)/2)  
...
If we take a look at the grafs. We can see that the homicide rate has been deescalating, especially in Italy, but in places like England the homicide rate has not been going down as drasticly as in Italy. In Scandinavia the rate has been going down steadily since the 15th century. Which can have something to do with the fact that the monarchs have had a longer reign period which can be a sign that the longer a monarch have ruled the more stability and order they have given to society. So are we more civilized today? If we look at the grafs I would say that we are, as law and order has been more widespread and has been suppressing the need for homicide instances.